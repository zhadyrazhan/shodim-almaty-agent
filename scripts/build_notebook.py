...
если GPU нет, ячейки ниже сами себя пропустят."""),
    code("""# Если сессия открыта давно, ячейки этого ноутбука могли устареть
# относительно main, даже если клонированная папка на диске свежая:
# `!python training/...` ниже подтянет актуальный код, а вот код, написанный
# прямо в ячейках 5.2/5.4, — нет, он живёт в этом .ipynb, а не на диске.
# git pull тут не панацея (открытые ячейки он не перезапишет — для этого
# нужно заново открыть ноутбук из GitHub), но хотя бы держит на диске в
# синхроне то, что реально запускается через `!python ...`.
import subprocess

subprocess.run(["git", "pull", "-q", "origin", "main"])
